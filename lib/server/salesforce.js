...
export const getContactLitecardPassId = async (contactId) => {
  const connection = await getSalesforceConnection();
  const result = await connection.sobject("Contact").retrieve(contactId, ["Litecard_Pass_ID__c"]);
  const passId = result?.Litecard_Pass_ID__c;
