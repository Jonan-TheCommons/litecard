...
export const getPass = async (passId) => {
  const { litecardBaseUrl } = getServerConfig();
  const token = await generateAccessToken();

  const response = await fetch(`${litecardBaseUrl}/card/${passId}`, {
    method: "GET",
    headers: {
      accept: "application/json",
  ...
  if (!response.ok) {
    throw new Error(`Litecard get pass failed: ${response.status} ${response.statusText}`);
  }
