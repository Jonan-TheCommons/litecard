...
export default function FreeCoffeeDashboard() {
  ...
  useEffect(() => {
    ...
    const loadServerConfig = async () => {
      try {
        const response = await fetch("/api/free-coffee/process", {
          cache: "no-store",
          method: "GET",
        });
        ...
        <article className="panel panel-dark">
          <div className="panel-body">
            ...
            <p className="helper-copy">
              For each member, checks Salesforce for an existing Litecard pass. If found, fetches the existing wallet
              links and sends a resend email. If not found, creates a new pass and sends the loyalty email.
            </p>
        ...
        <article className="panel">
          <div className="panel-body form-panel">
            <div className="toolbar">
              <div>
                <h2>Free coffee intake</h2>
                <p className="helper-copy">
                  Enter members manually or upload a CSV. Each member is checked for an existing Litecard pass before
                  processing. Large CSV files are processed in chunks of up to {batchChunkSize} records.
                </p>
              </div>
            </div>
                ...
                <section className="csv-upload-panel">
                  <div className="csv-upload-header">
                    <div>
                      ...
                      <p className="helper-copy">
                        Upload a CSV with member records. Extra columns are ignored, but the required headers must still
                        be present.
                      </p>
                    </div>
                  ...
                  {hasCsvUpload ? (
                    ...
                    <div className="empty-state">No CSV loaded yet. Choose a file to process members.</div>
                  )}
                </section>
              </section>
            )}
            ...
            <div className="panel-body">
              <div className="results-modal-header">
                <div>
                  ...
                  <p className="helper-copy">
                    Successful records show whether a new pass was created or an existing one was reused, along with
                    wallet links. Failed records return the error for retry.
                  </p>
                ...
                {results.results.map((result) => (
                  <article className="result-card" key={`${result.index}-${result.input.email}`}>
                    <header>
                      ...
                      <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                        {result.status === "success" ? (
                          <span className="status-chip chip-info">
                            {result.output.passCreated ? "New pass" : "Existing pass"}
                          </span>
                        ) : null}
                    ...
                    {result.status === "success" ? (
                      <>
                        <p className="meta-copy">
                          {result.output.passCreated
                            ? "New Litecard pass created. Salesforce updated and loyalty email sent."
                            : "Existing Litecard pass found. Resend email sent with current wallet links."}
                        </p>
                        <div className="result-meta">
                          <span>
                            {result.output.passCreated
                              ? `Card ID: ${result.output.pass.cardId}`
                              : `Pass ID: ${result.output.pass.id}`}
                          </span>
